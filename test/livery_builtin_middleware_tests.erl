...
body_limit_passes_when_under_cap_test() ->
    ...
    Cap = livery_test_adapter:run(
        [{livery_body_limit, #{max => 1024}}], Handler,
        #{body => {buffered, <<"hello">>}}),
...
body_limit_rejects_over_cap_test() ->
    ...
    Cap = livery_test_adapter:run(
        [{livery_body_limit, #{max => 4}}], Handler,
...
body_limit_passes_streaming_body_through_test() ->
    ...
    Cap = livery_test_adapter:run(
        [{livery_body_limit, #{max => 1}}], Handler,
        #{body => {stream, fake}}),
    ?assertEqual(200, livery_test_adapter:status(Cap)).
    ...
    try
        ...
        receive
            {log_event, Event} ->
                ...
                ?assertMatch({report, #{msg := "livery_access",
                                        method := <<"POST">>,
                                        path := <<"/foo">>,
                                        status := 200}}, Msg)
        after 200 ->
...
body_limit_with_iolist_body_test() ->
    ...
    Handler = fun(_R) -> error(must_not_be_called) end,
    Cap = livery_test_adapter:run(
        [{livery_body_limit, #{max => 3}}], Handler,
